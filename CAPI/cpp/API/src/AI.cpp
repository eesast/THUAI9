...
namespace
{
    ...
    constexpr int32_t kMaxCharacters = 5;
    constexpr int32_t kBuildCost = 50;
    constexpr int32_t kBuildCooldown = 50;
    constexpr int32_t kProduceCooldown = 20;
    constexpr int32_t kTechThreshold = 200;
    constexpr int32_t kAttackPhaseTimeMs = 120'000;
    ...
    struct CharState {
        ...
        static constexpr int32_t kOscillationHistory = 8;
        int32_t posHistoryX[kOscillationHistory]{};
        int32_t posHistoryY[kOscillationHistory]{};
        int32_t posHistoryIdx = 0;
        int32_t lastActionFrame = 0;
    ...
    struct TeamState {
        ...
        int32_t lastProduceFailFrame = -1000;
        int32_t lastBuildFrame = -1000;
        int32_t lastBuildFailFrame = -1000;
        int32_t lastTechFrame = -1000;
        int32_t nextPlayerID = 2;
        int32_t roleAssignCounter = 0;
    };
    ...
    bool TryTradeFromFactory(ICharacterAPI& api, const THUAI9::Character& self, CharState& s) {
        ...
        if (atFactory) {
            ...
            s.taskRetries = 0;
            s.lastFactoryEmptyFrame = api.GetFrameCount();
            return false;
        }

        // 不在工厂旁：如果刚查过没货，冷却期内不回去
        if (api.GetFrameCount() - s.lastFactoryEmptyFrame < 100)  // ~5 秒
            return false;
    ...
    void AttackerCharAI(ICharacterAPI& api, const THUAI9::Character& self, CharState& s) {
        if (s.phase == Phase::SeekEnemy) {
            ...
            // 侦查
            api.Print("attacker: no target, scouting");
            MoveToRandomCell(api, self, s);
            s.lastActionFrame = api.GetFrameCount();
            return;
        }
    ...
    void CharAI(ICharacterAPI& api, const THUAI9::Character& self, CharState& s) {
        ...
        FindSomethingToDo(api, self, s);
        NoActionHeartbeat(api, self, s);
    }
