...
#undef ABSL_FLAGS_INTERNAL_DFLT_FOR_TYPE
        ...
        using FlagUseValueAndInitBitStorage =
            std::integral_constant<bool, std::is_trivially_copyable<T>::value && std::is_default_constructible<T>::value && (sizeof(T) < 8)>;

        template<typename T>
        using FlagUseOneWordStorage =
            std::integral_constant<bool, std::is_trivially_copyable<T>::value && (sizeof(T) <= 8)>;
        ...
        public:
            ...
            static constexpr int RequiredAlignment()
            {
                return 4;
            }
        ...
        struct FlagValue<T, FlagValueStorageKind::kValueAndInitBit> : FlagOneWordValue
        {
            ...
            bool Get(const SequenceLock&, T& dst) const
            {
                ...
                if (ABSL_PREDICT_FALSE(storage == 0))
                {
                    ...
                    static_assert(offsetof(FlagValueAndInitBit<T>, init) == sizeof(T), "Unexpected memory layout of FlagValueAndInitBit");
                    return false;
                }
        ...
        template<typename T>
        struct FlagValue<T, FlagValueStorageKind::kHeapAllocated> : FlagMaskedPointerValue
        {
...
#pragma GCC diagnostic ignored "-Wnon-virtual-dtor"
        ...
        {
        public:
            constexpr FlagImpl(const char* name, const char* type_name, const char* filename, FlagOpFn op, FlagHelpArg help, FlagValueStorageKind value_kind, FlagDefaultArg default_arg) :
                name_(name),
...
#pragma GCC diagnostic pop
        ...
        {
        public:
            constexpr Flag(const char* name, const char* type_name, const char* filename, FlagHelpArg help, const FlagDefaultArg default_arg) :
                impl_(name, type_name, filename, &FlagOps<T>, help, flags_internal::StorageKind<T>(), default_arg),
                value_()
            {
            }
        ...
        public:
            ...
            // temporary.
            constexpr operator FlagRegistrarEmpty() const
            {
                return {};
            }  // NOLINT
