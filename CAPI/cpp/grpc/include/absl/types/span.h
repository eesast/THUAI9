...
namespace absl
{
    ...
    public:
        ...
        constexpr Span(pointer array ABSL_ATTRIBUTE_LIFETIME_BOUND, size_type length) noexcept
            :
            ptr_(array),
            len_(length)
        {
        ...
        constexpr Span(T(  // NOLINT(google-explicit-constructor)
            &a ABSL_ATTRIBUTE_LIFETIME_BOUND
        )[N]) noexcept
            :
            Span(a, N)
        {
        ...
        ) noexcept  // NOLINT(runtime/references)
            :
    ...
    //
    template<int&... ExplicitArgumentBarrier, typename T>
    constexpr Span<const T> MakeConstSpan(absl::Nullable<T*> ptr, size_t size) noexcept
    {
        return Span<const T>(ptr, size);
    }

    template<int&... ExplicitArgumentBarrier, typename T>
    Span<const T> MakeConstSpan(absl::Nullable<T*> begin, absl::Nullable<T*> end) noexcept
    {
        ABSL_HARDENING_ASSERT(begin <= end);
