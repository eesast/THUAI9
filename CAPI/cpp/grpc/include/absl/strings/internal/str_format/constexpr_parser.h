    ...
    namespace str_format_internal
    {
        ...
        struct UnboundConversion
        {
            ...
            public:
                ...
                constexpr bool is_from_arg() const
                {
                    return value_ < -1;
                }
        ...
        public:
            ...
            constexpr bool is_conv() const
            {
                return (tag_ & 0x80) == 0;
            }
            constexpr bool is_length() const
            {
                return (tag_ & 0xC0) == 0x80;
            }
            constexpr bool is_flags() const
            {
        ...
        constexpr bool CheckFastPathSetting(const UnboundConversion& conv)
        {
            ...
            if (width_precision_needed && conv.flags == Flags::kBasic)
            {
                ...
                fprintf(stderr, "basic=%d left=%d show_pos=%d sign_col=%d alt=%d zero=%d "
                                "width=%d precision=%d\n",
                        ...
                        FlagsContains(conv.flags, Flags::kZero) ? 1 : 0,
                        conv.width.value(),
        ...
        constexpr const char* ConsumeConversion(const char* pos, const char* const end, UnboundConversion* conv, int* next_arg)
        {
...
#undef ABSL_FORMAT_PARSER_INTERNAL_GET_CHAR
        ...
        constexpr const char* ConsumeUnboundConversion(const char* p, const char* end, UnboundConversion* conv, int* next_arg)
        {
            if (*next_arg < 0)
