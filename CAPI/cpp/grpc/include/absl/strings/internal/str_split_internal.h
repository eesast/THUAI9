    ...
    namespace strings_internal
    {
            ...
            SplitIterator& operator++()
            {
                do
                {
                    ...
                    if (d.data() == text.data() + text.size())
                        state_ = kLastState;
                    curr_ = text.substr(pos_, static_cast<size_t>(d.data() - (text.data() + pos_)));
        ...
        struct ShouldUseLifetimeBound<
            ...
            std::enable_if_t<
                ...
                std::is_same<typename Container::value_type, absl::string_view>::value>> : std::true_type
        {
        };
        ...
        public:
            ...
            template<
                typename Container,
                std::enable_if_t<ShouldUseLifetimeBound<StringType, Container>::value && SplitterIsConvertibleTo<Container>::value, std::nullptr_t> = nullptr>
            ...
            // corresponding value.
            template<typename First, typename Second, std::enable_if_t<ShouldUseLifetimeBoundForPair<StringType, First, Second>::value, std::nullptr_t> = nullptr>
            // NOLINTNEXTLINE(google-explicit-constructor)
            ...
            // string is used.
            template<typename ElementType, std::size_t Size, std::enable_if_t<ShouldUseLifetimeBoundForArray<StringType, ElementType, Size>::value, std::nullptr_t> = nullptr>
            // NOLINTNEXTLINE(google-explicit-constructor)
            ...
            struct ConvertToContainer<std::vector<absl::string_view, A>, absl::string_view, false>
            {
                ...
                ) const
                {
                    ...
                    for (auto it = splitter.begin(); !it.at_end();)
                    {
                        ...
                        v.insert(v.end(), ar.begin(), ar.begin() + static_cast<ptrdiff_t>(index));
                    }
                    return v;
                }
            };
