...
namespace absl
{
    ...
    namespace random_internal
    {
        ...
        private:
            static absl::string_view TagLbBound(IntervalClosedOpenTag)
            {
                return "[";
            }
            static absl::string_view TagLbBound(IntervalOpenOpenTag)
            {
                return "(";
            }
            static absl::string_view TagLbBound(IntervalClosedClosedTag)
            {
                return "[";
            }
            static absl::string_view TagLbBound(IntervalOpenClosedTag)
            {
                return "(";
            }
            static absl::string_view TagUbBound(IntervalClosedOpenTag)
            {
                return ")";
            }
            static absl::string_view TagUbBound(IntervalOpenOpenTag)
            {
                return ")";
            }
            static absl::string_view TagUbBound(IntervalClosedClosedTag)
            {
                return "]";
            }
            static absl::string_view TagUbBound(IntervalOpenClosedTag)
            {
                return "]";
            }

            template<typename TagType>
            static void ValidateImpl(std::true_type /* is_floating_point */, NumType x, TagType tag, NumType lo, NumType hi)
            {
                ...
                ABSL_INTERNAL_CHECK(lb <= x && x < ub, absl::StrCat(x, " is not in ", TagLbBound(tag), lo, ", ", hi, TagUbBound(tag)));
            }

            template<typename TagType>
            static void ValidateImpl(std::false_type /* is_floating_point */, NumType x, TagType tag, NumType lo, NumType hi)
            {
                using stream_type =
                ...
                ABSL_INTERNAL_CHECK(
                    lb <= x && x <= ub,
                    absl::StrCat(stream_type{x}, " is not in ", TagLbBound(tag), stream_type{lo}, ", ", stream_type{hi}, TagUbBound(tag))
                );
            }
        };
