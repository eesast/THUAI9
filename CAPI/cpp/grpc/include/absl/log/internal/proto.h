...
namespace absl
{
    ...
    namespace log_internal
    {
        ...
        // Consumes up to kMaxVarintSize * 2 bytes (20).
        inline bool EncodeVarintZigZag(uint64_t tag, int64_t value, absl::Span<char>* buf)
        {
            if (value < 0)
        ...
        // Used for string, bytes, message, and packed-repeated field type.
        // Consumes up to kMaxVarintSize * 2 + value.size() bytes (20 + value.size()).
        inline bool EncodeString(uint64_t tag, absl::string_view value, absl::Span<char>* buf)
        {
        ...
        // Used for string, bytes, message, and packed-repeated field type.
        // Consumes up to kMaxVarintSize * 2 + value.size() bytes (20 + value.size()).
        inline bool EncodeStringTruncate(uint64_t tag, absl::string_view value, absl::Span<char>* buf)
        {
        ...
        // Used for string, bytes, message, and packed-repeated field type.
        // Consumes up to kMaxVarintSize * 2 bytes (20).
        ABSL_MUST_USE_RESULT absl::Span<char> EncodeMessageStart(uint64_t tag, uint64_t max_size, absl::Span<char>* buf);
        ...
        {
            return size >= 10 ? (std::numeric_limits<uint64_t>::max)() : (static_cast<uint64_t>(1) << size * 7) - 1;
        }
        ...
        public:
            ...
            {
                return absl::bit_cast<double>(value_);
            }
            ...
            {
                if (value_ % 2)
                    return static_cast<int32_t>(0 - ((value_ - 1) / 2) - 1);
                return static_cast<int32_t>(value_ / 2);
            }
            int64_t sint64_value() const
            {
                if (value_ % 2)
                    return 0 - ((value_ - 1) / 2) - 1;
                return value_ / 2;
            }
            ...
            absl::Span<const char> bytes_value() const
            {
                return data_;
            }
