...
#pragma GCC diagnostic pop
            ...
            template<TargetType target_type, class QualDecayedTRef, class F, absl::enable_if_t<target_type == TargetType::kCompatibleAnyInvocable, int> = 0>
            void Initialize(F&& f)
            {
            ...
            template<TargetType target_type, class QualDecayedTRef, class F, absl::enable_if_t<target_type == TargetType::kIncompatibleAnyInvocable, int> = 0>
            void Initialize(F&& f)
            {
                if (f.HasValue())
                {
...
#define ABSL_INTERNAL_ANY_INVOCABLE_IMPL_(cv, ref, inv_quals, noex)                                                                                  \
    ...
    public:                                                                                                                                          \
        ...
        using CallIsNoexceptIfSigIsNoexcept =                                                                                                        \
            TrueAlias<ABSL_INTERNAL_ANY_INVOCABLE_NOEXCEPT_CONSTRAINT(inv_quals, noex)>;                                                             \
                                                                                                                                                     \
