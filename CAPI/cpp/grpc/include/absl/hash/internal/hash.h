...
namespace absl
{
    ...
    namespace hash_internal
    {
        ...
        template<typename Char, typename H, typename = absl::enable_if_t<std::is_same<Char, wchar_t>::value || std::is_same<Char, char16_t>::value || std::is_same<Char, char32_t>::value>>
...
#define ABSL_INTERNAL_STD_FILESYSTEM_PATH_HASH_AVAILABLE 1
        ...
        template<typename Path, typename H, typename = absl::enable_if_t<std::is_same_v<Path, std::filesystem::path>>>
...
#define ABSL_HASH_INTERNAL_SUPPORT_LEGACY_HASH_ 0
        ...
        class ABSL_DLL MixingHashState : public HashStateBase<MixingHashState>
        {
            ...
            static constexpr uint64_t kMul =
                sizeof(size_t) == 4 ? uint64_t{0xcc9e2d51} : uint64_t{0xdcb22ca68cb134ed};

            template<typename T>
            using IntegralFastPath =
                conjunction<std::is_integral<T>, is_uniquely_represented<T>, FitsIn64Bits<T>>;
      ...
      inner_state = MixingHashState{}; });
            ...
            static MixingHashState combine_raw(MixingHashState hash_state, uint64_t value)
            {
            ...
            )
            {
                ...
                const uint64_t m1 = Mix(Read8(tail_16b_ptr) ^ kStaticRandomData[3], Read8(tail_16b_ptr + 8) ^ state);
                return m0 ^ m1;
            }
    ...
    (!defined(__apple_build_version__) ||            \
     __apple_build_version__ >= 19558921)  // Xcode 12
        ...
        )
        {
            ...
            {
                return Mix(state ^ hash_internal::CityHash32(reinterpret_cast<const char*>(first), len), kMul);
            }
