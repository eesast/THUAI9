...
#define ABSL_BTREE_ENABLE_GENERATIONS
        ...
        private:
            ...
            constexpr static auto InternalLayout()
            {
                return internal_layout_type();
            }
        ...
        public:
            ...
            btree_node* child(field_type i) const
            {
                return GetField<4>()[i];
            }
            ...
            btree_node*& mutable_child(field_type i)
            {
                return GetField<4>()[i];
            }
            ...
            SearchResult<size_type, false> linear_search_impl(
                const K& k, size_type s, const size_type e, const Compare& comp, std::false_type /* IsCompareTo */
            ) const
            {
                while (s < e)
                {
            ...
            SearchResult<size_type, true> linear_search_impl(
                const K& k, size_type s, const size_type e, const Compare& comp, std::true_type /* IsCompareTo */
            ) const
            {
                while (s < e)
                {
            ...
            SearchResult<size_type, false> binary_search_impl(
                const K& k, size_type s, size_type e, const Compare& comp, std::false_type /* IsCompareTo */
            ) const
            {
                while (s != e)
                {
            ...
            SearchResult<size_type, true> binary_search_impl(
                const K& k, size_type s, size_type e, const CompareTo& comp, std::true_type /* IsCompareTo */
            ) const
            {
            ...
            {
                if (std::is_base_of<BtreeTestOnlyCheckedCompareOptOutBase, Compare>::value ||
                ...
                for (field_type j = start(); j < finish(); ++j)
                {
                    ...
                    if (new_cmp < cmp || (!kCanHaveEquivKeys && new_cmp == 0))
                        return false;
                    cmp = new_cmp;
                }
                return true;
            ...
            void rebalance_right_to_left(field_type to_move, btree_node* right, allocator_type* alloc);
            void rebalance_left_to_right(field_type to_move, btree_node* right, allocator_type* alloc);
            ...
            void init_leaf(field_type position, field_type max_count, btree_node* parent)
            {
                set_generation(0);
                set_parent(parent);
        ...
        private:
            ...
            {
                next_generation();
                for (slot_type *src = src_node->slot(src_i + n), *end = src - n, *dest = slot(dest_i + n);
                     src != end;
                     --src, --dest)
                {
        ...
        bool AreNodesFromSameContainer(const Node* node_a, const Node* node_b)
        {
            ...
            if (node_a == nullptr || node_b == nullptr)
                return true;
            while (!node_a->is_root())
                node_a = node_a->parent();
            while (!node_b->is_root())
        ...
        public:
            ...
            btree_iterator(Node* n, int p) :
                btree_iterator_generation_info(n != nullptr ? n->generation() : ~uint32_t{}),
                node_(n),
                position_(p)
            {
            }
        ...
        private:
            ...
            {
                ABSL_HARDENING_ASSERT(((node_ == nullptr && other.node_ == nullptr) || (node_ != nullptr && other.node_ != nullptr)) && "Comparing default-constructed iterator with "
                ...
                assert(AreNodesFromSameContainer(node_, other.node_) && "Comparing iterators from different containers.");
                assert_valid_generation(node_);
            ...
            bool IsEndIterator() const
            {
                ...
                while (!node->is_root())
                {
                    if (node->position() != node->parent()->finish())
                        return false;
                    node = node->parent();
                }
                return true;
        ...
        template<typename P>
        template<typename... Args>
        inline void btree_node<P>::emplace_value(const field_type i, allocator_type* alloc, Args&&... args)
        {
            assert(i >= start());
            assert(i <= finish());
        ...
        template<typename P>
        void btree_node<P>::rebalance_right_to_left(field_type to_move, btree_node* right, allocator_type* alloc)
        {
        ...
        template<typename P>
        void btree_node<P>::rebalance_left_to_right(field_type to_move, btree_node* right, allocator_type* alloc)
        {
            assert(parent() == right->parent());
            assert(position() + 1 == right->position());
            ...
            -> difference_type
        {
            ...
            assert(begin.node_ != end.node_ || !begin.node_->is_leaf() || begin.position_ != end.position_);
        ...
        {
            iter.node_->value_destroy(static_cast<field_type>(iter.position_), mutable_allocator());
            ...
            else
            {
                ...
                iter.node_->transfer_n(num_to_transfer, static_cast<size_type>(iter.position_), transfer_from, iter.node_, mutable_allocator());
            }
            ...
            -> std::pair<size_type, iterator>
        {
            ...
            while (size_ > target_size)
            {
                if (begin.node_->is_leaf())
                {
                    ...
                    begin.node_->remove_values(static_cast<field_type>(begin.position_), to_erase, mutable_allocator());
                    size_ -= to_erase;
            ...
            -> iterator
        {
            ...
            const auto transfer_and_delete = [&](node_type* old_node,
                                                 node_type* new_node)
            {
                new_node->transfer_n(old_node->count(), new_node->start(), old_node->start(), old_node, alloc);
    ...
    defined(ABSL_HAVE_HWADDRESS_SANITIZER)
            if (!replaced_node)
            {
                ...
                else
                {
                    ...
                    if (was_rightmost)
                        mutable_rightmost() = new_node;
                    // The leftmost node is stored as the parent of the root node.
                    if (was_leftmost)
                        root()->set_parent(new_node);
                }
            }
#endif
            iter.node_->emplace_value(static_cast<field_type>(iter.position_), alloc, std::forward<Args>(args)...);
            assert(
                iter.node_->is_ordered_correctly(static_cast<field_type>(iter.position_), original_key_compare(key_comp())) &&
        ...
        typename btree<P>::size_type btree<P>::internal_verify(
            const node_type* node, const key_type* lo, const key_type* hi
        ) const
        {
            assert(node->count() > 0);
            assert(node->count() <= node->max_count());
