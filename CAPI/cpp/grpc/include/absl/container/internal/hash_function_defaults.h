...
namespace absl
{
    ...
    namespace container_internal
    {
        ...
        {
            using is_transparent = void;
            bool operator()(std::basic_string_view<TChar> lhs, std::basic_string_view<TChar> rhs) const
            {
                return lhs == rhs;
            }
        };
        ...
        template<typename T>
        struct HasAbslContainerHash<T, absl::void_t<typename T::absl_container_hash>> : std::true_type
        {
        };
        ...
        template<typename T>
        struct HasAbslContainerEq<T, absl::void_t<typename T::absl_container_eq>> : std::true_type
        {
        };
        ...
        struct AbslContainerEq<
            T,
            typename std::enable_if_t<HasAbslContainerEq<T>::value>>
        {
        ...
        struct AbslContainerHash<
            T,
            typename std::enable_if_t<HasAbslContainerHash<T>::value>>
        {
        ...
        struct HashEq<T, typename std::enable_if_t<HasAbslContainerHash<T>::value>>
        {
            ...
            static_assert(IsTransparent<Hash>::value, "absl_container_hash must be transparent. To achieve it add a "
                                                      "`using is_transparent = void;` clause to this type.");
            static_assert(IsTransparent<Eq>::value, "absl_container_eq must be transparent. To achieve it add a "
                                                    "`using is_transparent = void;` clause to this type.");
        };
