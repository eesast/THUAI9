...
namespace absl
{
    ...
    //  }
    template<class T, class Hash = DefaultHashContainerHash<T>, class Eq = DefaultHashContainerEq<T>, class Alloc = std::allocator<T>>
    class ABSL_ATTRIBUTE_OWNER node_hash_set : public absl::container_internal::raw_hash_set<absl::container_internal::NodeHashSetPolicy<T>, Hash, Eq, Alloc>
    {
    ...
    void swap(node_hash_set<T, H, E, A>& x, node_hash_set<T, H, E, A>& y) noexcept(noexcept(x.swap(y)))
    {
        return x.swap(y);
    }
    ...
    namespace container_internal
    {
        ...
        decay_t<Function> c_for_each_fast(const node_hash_set<T, H, E, A>& c, Function&& f)
        {
