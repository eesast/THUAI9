...
#pragma GCC diagnostic pop
        ...
        iterator insert(const_iterator pos, ForwardIterator first, ForwardIterator last) ABSL_ATTRIBUTE_LIFETIME_BOUND
        {
            ...
            if (ABSL_PREDICT_TRUE(first != last))
            {
                return storage_.Insert(
                    pos, IteratorValueAdapter<A, ForwardIterator>(first), static_cast<size_type>(std::distance(first, last))
                );
            }
            else
            {
        ...
        iterator emplace(const_iterator pos, Args&&... args) ABSL_ATTRIBUTE_LIFETIME_BOUND
        {
            ABSL_HARDENING_ASSERT(pos >= begin());
            ABSL_HARDENING_ASSERT(pos <= end());
