...
| 要素 | 说明 |
|------|------|
| 地图 | 可配置网格（easy 5×5 / medium 10×10 / hard 15×15），含障碍/市场/资源/算力中心 |
| 产品 | 5 类（半导体/药品/小商品/服饰/食品），正弦价格函数，支持工厂生产链 |
| 动作 | 28 个离散动作：基础(6) + 卖出×5 + 生产链(8) + 科技(8) + 占领(1) |
| 奖励 | 销售收益（主）+ 采集塑形 + 算力中心解锁 + 时间惩罚 + 无效惩罚 |
| 得分 | 销售额 × 10 |
| 科技 | 8 项科技（降低成本/效率/营销/耐久/多产线/路径优化/市场分析/算力扩张） |

详细规则参见 [CONTESTANT_GUIDE.md](docs/CONTESTANT_GUIDE.md)。
