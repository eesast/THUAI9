...
echo [THUAI9] Launching UI, server, and debug Python clients...
...
start "THUAI9 Python 1-0" /D "%PY_ROOT%" cmd /k ""%PYTHON_EXE%" -m PyAPI.main -t 1 -p 0 -I %SERVER_IP% -P %SERVER_PORT% --aiModule %ACTIVE_AI_MODULE% %PY_FLAGS%"
start "THUAI9 Python 2-0" /D "%PY_ROOT%" cmd /k ""%PYTHON_EXE%" -m PyAPI.main -t 2 -p 0 -I %SERVER_IP% -P %SERVER_PORT% --aiModule %DUMMY_AI_MODULE% %PY_FLAGS%"
