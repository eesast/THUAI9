...
public static class PixelAssetUnityBinder
{
    ...
    public static void RunAll()
    {
        ...
        AssignRegistryToMainGame(registry);
        ...
        Debug.Log($"[Assets] Done. Runtime registry sprites: {spriteRefs}, prefabs: {prefabRefs}. MainGame scene wired: {MainGameScenePath}");
    }
    ...
    private static void AssignRegistryToMainGame(PixelAssetRegistry registry)
    {
        if (registry == null || !File.Exists(MainGameScenePath))
        {
            return;
        }
        ...
        {
            Debug.LogWarning("[Assets] MainGame scene has no RenderManager to wire pixel asset registry.");
            return;
        }
